...
crontab -l 2>/dev/null | grep -v "cloudflare.py" | crontab - 2>/dev/null || true
...
# Create simple config file for cloudflare.py
...
(crontab -l 2>/dev/null; echo "* * * * * cd $SCRIPT_DIR && python3 cloudflare.py > /var/www/html/subscription.txt") | crontab -
...
cd "$SCRIPT_DIR" && python3 cloudflare.py > /var/www/html/subscription.txt
